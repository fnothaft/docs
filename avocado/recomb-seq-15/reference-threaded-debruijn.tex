...
This \emph{indexed de Bruijn} graph structure provides $\Omega(n)$ variant discovery with canonical
representations for most common variants. Finally, we demonstrate how an index de Bruijn graph
simplifies to an efficient algorithm for local sequence alignment.
...
This formulation of reassembly has an asymptotic cost bounded by \linebreak $\mathcal{O}(h r l_r
...
haplotypes, where realignment has complexity $\mathcal{O}(l_r l_h)$. We ignore the complexity of
enumerating the haplotype paths through the \emph{de Bruijn} graph here, but provide an extended
discussion of the complexity of path search in~\S\ref{sec:graph-complexity}.
...
may not be the best approach, and have included a longer discussion of options for processing spurs with
an indexed de Bruijn graph in~\S\ref{sec:spurs}.
...
To improve efficiency, we should emit statistics for genotyping \emph{while} traversing the graph. By doing
...
(where $l$ is typically much smaller than $n$). After describing this structure, we have demonstrated how
