...
generated for under \$1,000 in wetlab costs~\cite{nhgri}. Costs
...
which Figure~\ref{fig:bdg} depicts. We refer to the tools built on \textsc{ADAM}
...
To address these problems, we developed \textsc{ADAM}, a comprehensive framework
...
processing genomic data with \textsc{Spark SQL} extends the
...
\textsc{SoapSNP}~\cite{li09snp} variant caller.
...
the \textsc{GATK}~\cite{depristo11} using \textsc{Apache Hadoop}.
...
Support for \textsc{Pig} was implemented in two separate tools: \textsc{BioPig}~\cite{nordberg13}
...
analysis by enabling exploratory data analysis using \textsc{ADAM}'s APIs.
Thus, we needed a way to lower the latency of typical \textsc{Spark} queries.
The \textsc{Mango} project~\cite{tu16, morrow17} improved latency by introducing
...
\item The presentation layer~(6) provides datatype~(e.g., read/feature/variant)
  specific methods on top of the \textsc{GenomicRDD} API. This layer includes
...
Restricting records to be biallelic differs from \textsc{VCF}, which allows multiallelic records. By limiting
...
\textsc{ADAM}'s feature schema~(\S\ref{sec:feature-schema}) provides an abstracted view of a genomic feature
...
\textsc{Parquet} files from these other languages, it would have been prohibitive
...
The original BQSR engine in \textsc{ADAM} was generic and placed
...
tagged with the SAM \textsc{MD} tag. MD tags allow us to reconstruct the
...
coverage. However, as is also noted in the \textsc{SpeedSeq} paper~\cite{chiang15}, variant calls
...
This early version of \textsc{Avocado} performed the following process to score variants.
