...
\begin{abstract}
  ...
  a decoupled stack-based architecture that can accommodate many data formats,
...
\begin{acknowledgements}
  Throughout my time at Berkeley, I was fortunate to benefit from a fantastic
  set of mentors, peers, friends, and family.

  I would like to thank my advisors, David Patterson and Anthony Joseph. Dave
  and Anthony led the AMP Genomics project and were instrumental in defining
  a vision where best practices from software engineering and distributed
  systems could be brought to bear on complex genomics problems. Throughout my
  degree, I benefited tremendously from Dave and Anthony's experience and acumen.

  Most critically, Dave and Anthony assembled a fantastic team. I was fortunate
  to receive mentorship from both Kristal Curtis and Timothy Danford, which was
  critical in shaping my research and engineering interests. I also benefited
  tremendously from working with Alyssa Morrow, Michael Heuer, Justin Paschall,
  Devin Petersohn, Taner Dagdelen, and Lisa Wu. I hope that they will all
  recognize their influence within this dissertation. Additionally, I would be
  remiss to not thank Jon Kuroda and Shane Knapp, who did the critical and
  thankless work of managing all of the computational resources needed for the
  AMP Genomics project.

  Beyond Berkeley, we were fortunate to have close collaborative relationships
  with teams at the University of California, Santa Cruz and The Icahn School of
  Medicine at Mount Sinai. Most of all, I would like to thank Benedict Paten,
  who was critical in helping me straddle the junction of computer science and
  bioinformatics. I would also like to thank David Haussler, John Vivian, Hannes
  Schmidt, Beau Norgeot, and CJ Ketchum from UCSC, and Uri Laserson and Ryan
  Williams from Mount Sinai.

  All of the work done in this dissertation was released as open source
  software, and I greatly benefited from interacting with others in the open
  source community. The full list is much too long, but I would especially like
  to thank Neil Ferguson, Andy Petrella, Xavier Tordior, Deborah Siegel, and
  Denny Lee.

  I have also benefited from several mentors who I worked with prior to
  Berkeley, including William Dally, James Balfour, and Michael Linderman, who
  I worked with at Stanford, and Jacob Rael, who was my manager at Broadcom.
  I would especially like to thank Michael and Jacob. My time working with Jacob
  was my first experience driving a research project, and I frequently found
  opportunities to apply lessons I learned from Jacob to my research at Berkeley.
  After Stanford, Michael moved to Mount Sinai and then Middlebury University,
  and I have been able to work with him on both the ADAM and Deca projects. It
  has been a true pleasure to work with Michael again.

  Throughout the last few years, I have been fortunate to have Michael Pabst as
  a dear friend. A fellow software engineer, Michael has always been willing to
  lend an ear, whether I need to discuss an engineering problem, a research trend,
  or the oddities of academia. As a friend, Michael is a truly gifted listener,
  and I can always count on him for helpful and compassionate advice, witty
  banter, or a fascinating distraction at the end of a long day.
  
  I owe a significant debt to my family. My parents have always encouraged my
  academic interests and pursuits, and their support has been invaluable. My
  brothers Daniel and John have been in graduate school concurrent with myself,
  and it has been valuable to chat with them and to compare and contrast our
  different disciplines, programs, and experiences.

  Most importantly, I would like to thank my wife Iris and my daughter Rosalind.
  Iris inspired me to start my doctorate, and has been an incredible support
  throughout my whole time at Berkeley. Whether lending me an ear when I needed
  to talk through a research problem or helping me to cheer up during the
  inevitable doldrums that a graduate degree entails, I could have not done this
  without her besides me every step of the way. While Rose is still too young to
  have provided much feedback on my work, she made my doctorate much cuter than
  it would have been otherwise!

  Finally, I would like to thank my financial support throughout this degree. I
  was fortunate to have the support of an NSF Graduate Research Fellowship. My work
...
tractable. The dramatic improvement in sequencing cost since the Human
...
will continue to decrease for the foreseeable future, as sequencing vendors like
...
However, this scale of data solves biological problems at the cost of technical
...
by the rise of population scale sequencing. While the raw data from a single
...
architecture, we can eliminate architectural problems and provide better
...
datatypes, which provides a data-independent query model. These schemas form the
...
locations in the image between cycles, the ends of the reads are attached to
...
the captured image. Illumina platforms are susceptible to single base substitution
...
These error rates are then converted back into probabilities, which replace the
...
110 of the hours spent preprocessing the reads. Finally, implementations of these
...
of the Human Genome Project~\cite{lander01}. Informal specifications for
...
traverse a serial stream of data. The most prominent example of a genomic analysis
...
{Apache Hadoop Streaming} as a simple mechanism for parallelizing tools
...
made it easier for us accommodate the goals that evolved during the course of
the project.
...
evolved over the four years of the project. We discuss the specific trade-offs
...
\item The evidence access layer originally assumed that other query systems like
  ...
  de-emphasized this view. Specifically, once data has been loaded using the
...
\item The presentation layer~(6) was originally implemented by enhancing the
  ...
  out of the scope of expertise of the average bioinformatics programmer.
...
these design decisions. First, the schemas were originally fully de-normalized.
...
This metadata is string heavy, and benefited strongly from column-oriented
...
a reasonable assertion for row-oriented serialization techniques, this made it
...
\section{Accommodating Single-node Tools in {ADAM} With the {Pipe} API}
\label{sec:pipe-api}
...
erroneously during the sample preparation and polymerase chain reaction stages. Detection of duplicate reads
...
in the reads by looking for bubbles flanked by read versus reference sequence matches. In a colored de Bruijn
...
represents a sufficient improvement over the sum weighted edit distance between
...
To benchmark {Avocado}'s accuracy, we used the high coverage, PCR-free
...
using the Global Alliance for Genomics and Health's~(GA4GH's) benchmarking suite.
...
We processed the data through {Avocado}, the {GATK}'s
{HaplotypeCaller}~\cite{depristo11} (both GATK 3 and 4), SAMTools Mpileup/BCFtools
...
\hline
 & GATK3 & Avocado & Mpileup & FreeBayes \\
\hline
\hline
...
FreeBayes. We did not include the GATK4 in Table~\ref{tab:accuracy} because
of an issue with exporting to VCF that caused bands of records to be lost, which
falsely reduced recall. The precision of the GATK4's HaplotypeCaller is
comparable to the GATK3's HaplotypeCaller, with a 98.9\% SNP calling precision
and a 98.4\% INDEL calling precision.
...
the default lossless compression settings for each. BAM and CRAM files were generated
...
contains a comparison between BED and Parquet. BED is a generic file format for
...
evaluation on our internal cluster, which is comprised of 55 compute nodes, each with
two eight-core Intel Xeon E5-2670 CPUs with 2$\times$ hyperthreading, 256 GB RAM
and 4TB of HDD (four 1TB 7200RPM HDDs connected via 6Gpbs SATA). The nodes are
connected by a Gigabit ethernet network. In total, this
...
\begin{figure}[h]
  ...
  \caption{Disk and CPU metrics for alignment, preprocessing, coverage
    ...
    blue line is the maximum write bandwidth. The two dotted lines are stacked.
...
One of the disappointing outcomes of this work was that we were unable to
...
variants increases, it is no longer tractable to broadcast these variants, as
...
depicts the disk and processor utilization during the single sample extraction
...
lazy binding technique to the various genomic file formats. Additionally, we
...
be useful would be to time slice the statistics further, and to couple them with
...
such as JVM garbage collection epochs and statistics, network/disk/memory bandwith
...
caller that assumes a population structure, it is not compatible with somatic
...
One approach that may be tractable is to use a machine learning approach to run
...
To use this model to decrease consensus calling runtime, we would use the tool
...
and would run one or more variant callers that the trained model indicated would
...
to use the faster variant caller as our first pass tool. This representation of the
...
language is unclear. To enable an even higher performance gain, we should consider
...
arrays~(FPGAs) and application specific integrated circuits~(ASICs). The FPGA
...
to building ADAM as an open-source project. Over the course of the project, this
...
the Big Data Genomics project. While we greatly benefited from the open source
community, we think that we could have benefited even more. We discuss how we
...
utility. We would have benefited from reducing our scope down even further. If
...
single record read from disk versus serialized over the wire would vary by up to an
order of magnitude, which precipitated the year long refactor to our schemas
...
had disappointing performance in the real world on nested data. At times, we
...
implemented this so that a user would have provided us a region or list of regions
