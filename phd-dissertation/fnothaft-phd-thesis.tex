...
features as BED/GTF/GFF2/GFF3/\-{IntervalList}/\-{NarrowPeak}. If
...
which is explained by the large broadcast which runs during BQSR. This
introduces a serial several minute step, as the multiple gigabyte mask table
is broadcast to all of the nodes in the cluster.
...
\subsubsection{Architecture: Schemas and High Level APIs}
\label{sec:win-schemas-high-level-apis}

\subsubsection{Implementation: Latency}
\label{sec:win-latency}
...
\subsubsection{Project Management: Communication and Delivery}
\label{sec:fail-delivery}
...
\subsubsection{Implementation: Single Thread Performance}
\label{sec:fail-performance}
