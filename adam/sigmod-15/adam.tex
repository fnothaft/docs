...
%
\numberofauthors{1} 
\author{\alignauthor Frank~Austin~Nothaft$^{*}$, Matt~Massie$^{*}$, Timothy~Danford$^{*,\ddagger}$, Zhao~Zhang$^{*}$, Uri~Laserson$^{\circ}$, Carl~Yeksigian$^{\ddagger}$, Jey~Kottalam$^{*}$, Arun~Ahuja$^{\dagger}$, Jeff~Hammerbacher$^{\dagger, \circ}$, Michael~Linderman$^{\star}$, Michael~Franklin$^{*}$, Anthony~D.~Joseph$^{*}$, David~A.~Patterson$^{*}$ \\
...
and columnar storage techniques to achieve a $28\times$ end-to-end speedup over current genomics
pipelines, while reducing cost by 63\%. \linebreak From building this system, we were able to distill a set of
...
systems like \texttt{Impala} and \texttt{Spark-SQL} for data warehousing.
...
\texttt{Parquet}'s approach is preferable when compared to \texttt{Hadoop-BAM}~\cite{niemenmaa12}, a project that
supports the direct usage of legacy \texttt{BAM} files in \texttt{Hadoop}. \texttt{Hadoop-BAM} must pick splits, which
adds non-trivial overhead. Additionally, once \texttt{Hadoop-BAM} has picked a split, there is no guarantee that the split
is well placed. It is only guaranteed that the split position will not cause a functional error. Finally, although \texttt{BAM}
metadata is centralized, \texttt{Hadoop-BAM} punts on metadata distribution, and users must manually broadcast
the metadata.
