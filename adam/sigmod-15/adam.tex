...
\begin{abstract}
Revolutions in data acquisition are drastically changing how science conducts experiments. ``Next
...
motivate these principles with an example genomics pipeline that leverages open-source MapReduce
...
scientists routinely perform experiments that use terabytes~(TB) to \linebreak petabytes~(PB) of data.
...
non-experts, such as \texttt{Scikit-learn}~\cite{pedregosa11} and \texttt{MLI}~\cite{sparks13}.

The size of data is outgrowing computer performance. Using the current ``best practice'' genomic variant
calling pipeline~\cite{auwera13}, it takes approximately 120 hours to process a single, high-quality
human genome using a single, beefy node~\cite{talwalkar14}. To address these challenges, scientists
have started to apply computer systems techniques such as MapReduce~\cite{mckenna10, schatz09,
...
our systems. By choosing the correct commodity computing systems, we can provide better performance
and scalability than custom systems while enhancing the abstractions exposed to scientists.
...
This narrow waisted model is inspired by the OSI model for networked systems~\cite{zimmermann80}.
We demonstrate the generality of this model by using it to implement a system for processing astronomy
images, and achieve a $>3\times$ performance improvement over a state-of-the-art Message Passing
Interface~(MPI) based pipeline.
...
\begin{enumerate}
\item We characterize the important data and application patterns of emerging scientific domains, and
review why current scientific software does not fully satisfy these use cases.
\item We develop a modular, technology independent stack model to guide the design and
implementation of scientific processing and storage systems. We then demonstrate how this stack model
resolves both design anti-patterns \emph{and} functional bugs in current systems.
\item We implement a genomics pipeline that achieves a $50\times$ performance improvement and a
$2\times$ cost improvement over current ``best practice'' systems, and an astronomy image processing
pipeline that provides a $3\times$ improvement over the current ``state-of-the-art'' \linebreak MPI
system.
\item Finally, we illustrate how to provide coordinate-system guided processing in parallel across
shared-nothing systems. These traversals are necessary in many scientific applications.
\end{enumerate}

All of the software~(source code and executables) described in this paper is made available free of
charge under the permissive Apache 2 license.
...
example, a GATK \texttt{walker} is provided with a single view over the data~(a sorted iterator over a
...
that uses columnar storage techniques and special compression algorithms to achieve a 30\%
...
We compare the \texttt{Spark-mAdd} performance against the MPI based parallel implementation in
Montage v3.3 (\texttt{MPI-mAdd}). We performed the test on 1, 4, and 16 Amazon \texttt{cr3.8xlarge}
instances. We chose these instances because they provided very high performance networking, which is
necessary for good MPI performance. We use OrangeFS v2.8.8, a successor of PVFS~\cite{PVFS}, as
the shared file system when running \texttt{MPI-mAdd}. All 32 cores on each instance are used for both
\texttt{Spark-mAdd} and \texttt{MPI-mAdd}.
...
As shown in Figure~\ref{fig:madd-speedup}, \texttt{Spark-mAdd} runs 1.3x, 2.8x, 3.3x faster than
...
We demonstrate $>50\times$ performance improvements over conventional genomics processing
systems, along with linear strong scaling and a $2\times$ cost improvement. On the astronomy
workload, we achieve a $3\times$ speedup over the current best MPI-based solution. Additionally, while
