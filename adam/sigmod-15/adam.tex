...
%
\numberofauthors{1} 
\author{\alignauthor Frank~Austin~Nothaft$^{*}$, Matt~Massie$^{*}$, Timothy~Danford$^{*,\ddagger}$, Carl~Yeksigian$^{\ddagger}$, Jey~Kottalam$^{*}$, Arun~Ahuja$^{\dagger}$, Zhao~Zhang$^{*}$, Uri~Laserson$^{\circ}$, Jeff~Hammerbacher$^{\dagger, \circ}$, Michael~Linderman$^{\star}$, Michael~Franklin$^{*}$, Anthony~D.~Joseph$^{*}$, David~A.~Patterson$^{*}$ \\
$^{*}$\affaddr{Department of Electrical Engineering and Computer Sciences, University of California, Berkeley} \\
$^{\dagger}$\affaddr{Carl Icahn School of Medicine, Mount Sinai} \\
$^{\ddagger}$\affaddr{Genomebridge, Cambridge, MA} \\
$^{\circ}$\affaddr{Cloudera, San Francisco, CA} \\
\email{\{fnothaft, massie, tdanford, jey, zhaozhang, franklin, adj, pattrsn\}@berkeley.edu} \\
\email{\{arun.ahuja, jeff.hammerbacher, michael.linderman\}@mssm.edu} \\
\email{cyeksig@genomebridge.org} \\
\email{laserson@cloudera.com}
} 
...
In this paper, we demonstrate \texttt{ADAM}, an example genomics pipeline that leverages open-source MapReduce
and columnar storage techniques to achieve speedups unto $49\times$ over current genomics
pipelines, while reducing cost by 64\%. \linebreak From building this system, we were able to distill a set of
techniques for implementing scientific analyses efficiently using commodity ``big data'' systems. To demonstrate the
...
are not a universally great fit. Many genomics datasets are semistructured~(we make heavy use of optional and
repeated fields in our schemas) and are require complex UDFs to import into array databases. Other systems like
the Genome Query Language~\cite{kozanitis14} have extended SQL to provide efficient query semantics across
genomic coordinates. While GQL achieves performance improvements of up to 10$\times$ for certain algorithms,
...
techniques that can clarify the data model and access patterns. The seven layers of our stack model
...
\item {\bf Evidence Access} provides us with primitives for processing data, and allows us to
...
\item {\bf Application}s can use our evidence access and presentation layers to compose
...
the GATK) to perform some dimensionality reduction/statistical summarization of the data. This processing is
...
come from a single fragment of DNA, with an known approximate distance between each read.
...
\item \textbf{Background Matching} performs background removal on the
reprojected images. The best solution derived from the previous phase is used to smooth out the overlap between
tiled images.
\item \textbf{Tile Mosaicing} merges all corrected images into a aggregated mosaic file,
...
\hline
\texttt{ADAM} & 1$\dagger$ & 1602m & 366m & 143m & 108m & 2m17 \\
 & & $\sfrac{1}{1.25}\times$ & 1.7$\times$ & $\sfrac{1}{3.8}\times$ & $\sfrac{1}{1.3}\times$ & 2.7$\times$ \\
\hline
\texttt{ADAM} & 32$\star$ & 74m & 64m & 34m56 & 39m23 & 0m43 \\
 & & 17$\times$ & 10$\times$ & 1.2$\times$ & 2.1$\times$ & 8.6$\times$ \\
\hline
\texttt{ADAM} & 64$\star$ & 41m52 & 35m39 & 21m35 & 18m56 & 0m49 \\
 & & 30$\times$ & 18$\times$ & 2.0$\times$ & 4.3$\times$ & 7.5$\times$ \\
\hline
\texttt{ADAM} & 128$\star$ & 25m59 & 20m27 & 15m27 & 10m31 & 1m20 \\
 & & 49$\times$ & 32$\times$ & 2.9$\times$ & 7.9$\times$ & 4.3$\times$ \\
\end{tabular}
\end{center}
\end{table}
...
is more expensive than the best legacy tool (\texttt{SAMBAMBA}~\cite{sambamba}) for sorting and duplicate marking,
\texttt{ADAM} is less expensive for all other stages. In total, using \texttt{ADAM} reduces the end-to-end analysis cost
by 64\% over a pipeline constructed out of solely legacy tools.
...
\bf DM & \cite{sambamba} & \bf \$4.63 & 32$\star$ & \$13.04 \\
...
allows us to only load the input dataset a single time. Parquet also compresses the input and output data, which
reduces the volume of I/O performed. Additionally, MPI is bound by contention when trying to write all output to a single
...
guaranteed that the split position will not cause a functional error.
